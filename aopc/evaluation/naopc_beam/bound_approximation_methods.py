...
def get_pertubation_solver_callable(
    ...
    **kwargs,
    ...
    def mask_input(x, value_indices, word_map=None):
        ...
        return torch.where(mask == 1, x, torch.tensor(baseline_token_id))
    ...
    def get_prediction(input_ids, target_ids, device):
        ...
        for input_ids_batch in temp_dataloader:
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=True):
                ...
                out = (
                    torch.nn.functional.softmax(y_pred, dim=1)[:, target_ids]
                    .detach()
                    .cpu()
                )
                outs.append(out)
    ...
    def score_explanations(
        ...
        model_inputs = torch.cat(
            [
                mask_fn(input_ids, combination, word_map)
    ...
    def approx_pertubation_solver_callable(
        ...
        has_cls, has_eos = (
            (input_ids[0, 0] == cls_token_id).item(),
            (input_ids[0, -1] == eos_token_id).item(),
        )
        ...
        if preprocessing_step == "feature_attributions":
            ...
            ), "Attributions must be provided for feature attribution"
            ...
            weak_feature_importances_descending = sorted(
                weak_features, key=lambda x: x[1], reverse=False
            )
            weak_feature_importances_descending = {
                feature: -i
                for i, (feature, _) in enumerate(
                    weak_feature_importances_descending, start=1
                )
            }
            weak_feature_importances_ascending = sorted(
                weak_features, key=lambda x: x[1], reverse=True
            )
            weak_feature_importances_ascending = {
                feature: -i
                for i, (feature, _) in enumerate(
                    weak_feature_importances_ascending, start=1
                )
            }
            ...
            descending_beam = score_explanations(
                full_input_score,
                input_ids,
                [descending_explanation],
                target_ids,
                device,
                word_map=word_map,
            )
            ...
            ascending_beam = score_explanations(
                full_input_score,
                input_ids,
                [ascending_explanation],
                target_ids,
                device,
                word_map=word_map,
            )
        ...
        for _ in range(max_necessary_passes):
            ...
            ascending_split_index = next(
                i
                for i, e in enumerate(new_proposed_explanations)
                if e.descending == False
            )
        ...
        if preprocessing_step:
            best_descending_explanation.feature_importances.update(
                weak_feature_importances_descending
            )
