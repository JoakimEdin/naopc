...
for dataset_name in dataset_names:
    dataset_hf = datasets.load_dataset(
        "csv", data_files=f"data/{dataset_name}_test_short.csv", split="train"
    )
    ...
    number_of_forward_passes = (
        sum(dataset_hf.map(lambda x: {"2^n": math.pow(2, x["word_length"] - 2)})["2^n"])
        // BATCH_SIZE
        + 1
    )
    ...
    for model_name in model_names:
        ...
        dataset = PerturbDataset(
            dataset_hf,
            tokenizers[model_name].mask_token_id,
            tokenizers[model_name].pad_token_id,
            f"input_ids_{model_name}",
            word_map_callable=word_map_callable,
        )
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=BATCH_SIZE, num_workers=0, collate_fn=dataset.collate_fn
        )
        ...
        with torch.no_grad():
            for ids, token_key, word_key, input_ids_batch, attention_mask in track(
                dataloader, total=number_of_forward_passes, description=model_name
            ):
                logits = model(
                    input_ids_batch.to(device), attention_mask=attention_mask.to(device)
                ).logits.cpu()
