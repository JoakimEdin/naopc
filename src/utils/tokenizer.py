...
def get_word_map_callable(
    text_tokenizer: RobertaTokenizer | BertTokenizer | GPT2Tokenizer,
    include_space: bool = True,
    model_type: typing.Literal["roberta", "bert", "gpt2"] = "bert",
):
    ...
    def get_word_map_gpt2(
        input_ids: torch.Tensor,
    ) -> torch.Tensor:
        """Get a list mapping each token to a word. This is necessary because the GPT2 tokenizer splits words into subwords.
        The spaces are joined with the preceding word.
        Example:
        input_ids = [0, 1, 2, 3, 4, 5, 6, 7]
        ...
        Args:
            input_ids (torch.Tensor): Input ids to get word map for
            text_tokenizer (GPT2Tokenizer): Text tokenizer to use
...
if __name__ == "__main__":
    ...
    bert_callable = get_word_map_callable(bert_tokenizer, model_type="bert")
    gpt2_callable = get_word_map_callable(gpt2_tokenizer, model_type="gpt2")

    roberta_bert_mishaps = 0
    bert_gpt2_mishaps = 0
    roberta_gpt2_mishaps = 0
    total_mishaps = 0
    for example in yelp:
    ...
    print(f"Total pass: {len(yelp) - total_mishaps}")
    print(f"Roberta-Bert mishaps: {roberta_bert_mishaps}")
    print(f"Bert-GPT2 mishaps: {bert_gpt2_mishaps}")
    print(f"Roberta-GPT2 mishaps: {roberta_gpt2_mishaps}")
